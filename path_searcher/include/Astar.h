...
{	
	...
	protected:
		...
		//寻找Astar中某点的邻居点集
		void AstarGetSucc(GridNodePtr currentPtr, std::vector<GridNodePtr> & neighborPtrSets, std::vector<double> & edgeCostSets, std::vector<double> & angleCostSets);		
	...
	public:
		AstarPathFinder(std::string _distance,double _weight_a,double _weight_b,double _weight_angle,double length,double max_angle,double min_angle,int angle_resolution);
		~AstarPathFinder(){};
		//初始化点云地图
